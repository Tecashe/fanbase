  ...
  try {
    ...
    return NextResponse.json({
      ...
      rewards: creator.rewards.map((r) => ({
        ...
        points: `${(r.pointsRequired || 1500).toLocaleString()} PTS`,
        pointsValue: r.pointsRequired || 1500,
        cashValue: r.cashValue || (r.title.toLowerCase().includes('cash') ? 5000 : undefined),
        currency: r.currency || 'KES',
        meta: r.cashValue || r.title.toLowerCase().includes('cash') ? 'Cash Payout' : r.rewardType.toUpperCase(),
        icon: r.cashValue || r.title.toLowerCase().includes('cash') ? 'banknote' : r.rewardType === 'weekly' ? 'mic' : 'flame',
      })),
